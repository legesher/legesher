...
module.exports = grammar({
  ...
  rules: {
    ...
    // KEYWORD: FROM / IMPORT
    future_import_statement: $ =>
      seq('{from}', '__future__', '{import}', $._import_list),

    import_from_statement: $ =>
      seq(
        '{from}',
        choice($.relative_import, $.dotted_name),
        '{import}',
    ...
    // KEYWORD: ASSERT
    assert_statement: $ =>
      seq('{assert}', $._expression, repeat(seq(',', $._expression))),
    ...
    // KEYWORD: WHILE
    while_statement: $ =>
      seq('{while}', $._expression, ':', $._suite, optional($.else_clause)),
    ...
    except_clause: $ =>
      seq(
        '{except}',
        optional(
          seq($._expression, optional(seq(choice('{as}', ','), $._expression)))
        ),
        ':',
        $._suite
      ),
    ...
    // KEYWORD: ASYNC / WITH
    with_statement: $ =>
      seq(optional('{async}'), '{with}', commaSep1($.with_item), ':', $._suite),
    ...
    default_parameter: $ =>
      seq(choice($.identifier, $.keyword_identifier), '=', $._expression),
    ...
    // KEYWORD: EXEC / IN
    exec_statement: $ =>
      seq('{exec}', $.string, optional(seq('{in}', commaSep1($._expression)))),

    // KEYWORD: CLASS
    class_definition: $ =>
      seq('{class}', $.identifier, optional($.argument_list), ':', $._suite),
    ...
    decorator: $ =>
      seq('@', $.dotted_name, optional($.argument_list), $._newline),
    ...
    lambda: $ =>
      prec(
        PREC.lambda,
        seq('{lambda}', optional($.lambda_parameters), ':', $._expression)
      ),
    ...
    yield: $ =>
      seq(
        '{yield}',
        choice(seq('{from}', $._expression), optional($.expression_list))
      ),
    ...
    typed_parameter: $ =>
      prec(
        PREC.typed_parameter,
        seq(choice($.identifier, $.list_splat, $.dictionary_splat), ':', $.type)
      ),
    ...
    list_comprehension: $ =>
      seq('[', $._expression, $._comprehension_body, ']'),
    ...
    interpolation: $ =>
      seq('{', $._expression, optional($.type_conversion), '}'),
